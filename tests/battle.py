...
========================================
Test Batteries:
    1. Linguistic Complexity Test (2,000 Sanskrit/Pali derived edge-case words)
    2. Zero-Breakage Guarantee (HAL, ZWJ, modifier integrity)
    3. Glitched Token Detection (dead vocab scan + infinite-loop guard)
    4. Frontier Benchmarking Report (vs Gemini, GPT-4o, Llama 3)
    5. Round-Trip Consistency (decode(encode(text)) == text on 1M sentences)
    6. Boundary & Leading Space Edge-Cases (whitespace, mixed-script boundaries)

Usage:
    python battle_test.py --vocab_file output/vocab.json --test_file data/test.jsonl
    python battle_test.py --vocab_file output/vocab.json --test_file data/test.jsonl --full_corpus data/full_1m.jsonl
"""
...
# Optional dependencies for frontier models
try:
...
def _count_sinhala_words(text: str) -> int:
    return len(_SINHALA_WORD_RE.findall(text))

# Leading space marker (GPT-2 / SentencePiece convention)
...
# ═══════════════════════════════════════════════════════════════════════════════
# TOKENIZER WRAPPERS (For Battery 3)
# ═══════════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════════
# BATTERY 1: LINGUISTIC COMPLEXITY TEST (2,000 Sanskrit/Pali Edge-Case Words)
# ═══════════════════════════════════════════════════════════════════════════════
...
def _generate_complex_word_bank() -> list[tuple[str, str]]:
    ...
    while len(words) < 2000:
        c1 = random.choice(base_consonants)
        c2 = random.choice(base_consonants)
        pili = random.choice(pili_list)
        word = c1 + HAL + ZWJ + c2 + pili + random.choice(["ය", "ව", "න", ""])
        words.append((word, "filler_conjunct"))

    return words[:2000]


def _is_conjunct_internal(char: str) -> bool:
    """Returns True if this character should NEVER appear as the first
...
def test_linguistic_complexity(sgpe: SGPEEncoder) -> TestResult:
    """
    Battery 1: 
    Verify Layer 1 never splits consonant from HAL/ZWJ/modifiers.
    Verify atomic syllable tokenization even with leading space.
    """
    print("\n" + "=" * 80)
    print("BATTERY 1: LINGUISTIC COMPLEXITY TEST (2,000 Edge-Case Words)")
    print("=" * 80)
    ...
    for word, category in tqdm(word_bank, desc="  Layer1 integrity", unit=" word"):
        ...
        # Layer 1 tokenization (no BPE) — LinguisTrie syllabification only
        try:
        ...
        # Check: no token should START with a conjunct-internal character.
        # Skip degenerate categories where the word itself IS a bare modifier —
        # the tokenizer correctly handles them without crashing (no-crash is the pass).
        ...
        if category not in DEGENERATE_CATEGORIES:
            for i, tok in enumerate(l1_tokens):
                ...
                if _is_conjunct_internal(first_char) and i > 0:
                ...
                # Also check: token should not END with HAL+ZWJ unless it's
                # the last token (dangling ZWJ at split boundary)
        ...
        # Also test full pipeline (with BPE)
        try:
    ...
    # Also test with leading space applied
    ...
    details = (
        ...
        f"Avg L1 tokens/word: {avg_l1:.2f}, Avg BPE tokens/word: {avg_bpe:.2f}. "
...
def test_glitched_tokens(sgpe: SGPEEncoder, test_sentences: list[str]) -> TestResult:
    ...
    # Find zero-usage and near-zero tokens
    zero_usage = []
    ...
    for token_str, token_id in sgpe.vocab.items():
        ...
        if check_token:
            # 1. Check for mixed whitespace garbage (e.g. "\t:", "A ", "A\tB")
            # Valid patterns:
            # - Pure whitespace (e.g. " ", "\t", "   ")
            # - Leading space + non-whitespace (e.g. " A", " සි")
            # - No whitespace (e.g. "A", "සි")
            ...
            # 2. Check for bare ZWJ/HAL tokens (Data Artifacts)
            stripped = check_token.replace(ZWJ, '').replace(HAL, '').replace(' ', '').replace('\t', '').replace('\n', '')
            if not stripped:
                # Token contains only ZWJ, HAL, or whitespace
                # If it's pure whitespace, we already passed it above (mixed check passes pure).
                # If it has ZWJ/HAL, check if it's "Bare".
                has_zwj = ZWJ in check_token
                has_hal = HAL in check_token
                
                if has_zwj or has_hal:
                    # Bare modifier. Treat as Artifact (WARN), do NOT fail.
                    pass 

            # 3. Check glitched.py pattern
    ...
    # Test: try encoding each zero-usage token AS input to catch infinite loops
    ...
    for token_str, token_id in tqdm(zero_usage, desc="  stress-test", unit=" tok"):
        try:
            with time_limit(2.0):  # 2 second timeout per token
                encoded = sgpe.encode(token_str)
                decoded = sgpe.decode(encoded)
                # Also try encoding in sentence context
    ...
    # Separate critical failures from quality issues
    critical_issues = infinite_loop_tokens + crash_tokens + encode_errors
    
    # Determine status:
    # - FAIL if there are runtime issues OR glitched tokens 
    # - PASS otherwise
...
def test_frontier_benchmarking(
    ...
    """
    Battery 3: SOTA Tokenizer Comparison.

    Compares SGPE against official frontier tokenizers as of Feb 2026:
      - OpenAI o200k_base (GPT-4o, o3, o4) — local tiktoken, always runs
      - Gemini Flash — API, requires GEMINI_API_KEY (skipped in full_eval mode)
      - Claude Sonnet — API, requires ANTHROPIC_API_KEY
      - Llama 4 Scout — HuggingFace, requires HF_TOKEN + model access
      - DeepSeek V3.2 — HuggingFace, requires HF_TOKEN

    Capped at max_samples sentences to keep runtime reasonable.
    API-based tokenizers are rate-limited to avoid quota exhaustion.

    full_eval=True: skips API tokenizers, runs over ALL sentences.
    Use this mode for research paper citations — gives a stable, reproducible TWR.
    """
    print("\n" + "=" * 80)
    ...
    # ── Cap sentences (or use all for full_eval) ──
    if full_eval:
        sample = test_sentences  # all sentences — stable, reproducible
        print(f"\n  [FULL EVAL MODE] Using ALL {len(sample):,} sentences (local tokenizers only)")
        print(f"  This is the canonical number for research paper citation.")
    else:
    ...
    # 2. OpenAI o200k_base — local tiktoken, instant (GPT-4o, o3, o4, GPT-4.1)
    try:
    ...
    # 3. Gemini Flash — API-based, count_tokens only (official method)
    ...
    # 6. DeepSeek V3.2 — HuggingFace tokenizer only (public model)
    #    Requires HF_TOKEN (model is public but HF login recommended)
    if TRANSFORMERS_AVAILABLE and os.getenv("HF_TOKEN"):
        try:
    ...
    for text in tqdm(sample, desc="  benchmarking", unit=" sent"):
        n_words = _count_sinhala_words(text)
        n_chars = len(text)
    ...
    sample_words = ["ක්‍රෝෂ්ඨ්‍ර", "ශාස්ත්‍රීය", "ව්‍යාකරණය", "ප්‍රත්‍යක්ෂ", "ධම්මචක්කප්පවත්තන"]
    ...
    details = (
        ...
        + (" [PAPER-QUALITY: local tokenizers only, full corpus]" if full_eval else "")
    )
...
def test_roundtrip_consistency(
    ...
    # If we don't have 1M sentences, cycle the existing ones
    if len(sentences) < target_count:
...
def test_zero_breakage_extended(sgpe: SGPEEncoder) -> TestResult:
    """
    Battery 6: Extended zero-breakage tests.
    Verify that Layer 1 (LinguisTrie) NEVER splits:
      - Consonant from its HAL
      - HAL from its ZWJ
      - ZWJ from its following consonant
      - Consonant from its vowel sign (pili)
      - Consonant from its anusvara/visarga
    Across ALL possible Sinhala consonant combinations.
    """
    print("\n" + "=" * 80)
...
def main():
    ...
    parser.add_argument("--full_eval", action="store_true",
                        help="Full-corpus TWR eval: skip API tokenizers, use ALL sentences. "
                             "Use this for research paper citations (stable, reproducible).")
