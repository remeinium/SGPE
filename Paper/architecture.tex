...
\begin{enumerate}
\item \textbf{Syllabic initialization.} The base vocabulary is initialized with the atomic linguistic syllables and preserving passthrough characters emitted by LinguisTrie. For the target Abugida script, we inherently avoid blindly fracturing text into individual bytes or codepoints.
\item \textbf{Boundary-aware scoping.} Merges are performed only within word spans delimited by whitespace or non-Sinhala passthrough tokens, preventing cross-word tokens that would violate morphological boundaries.
\item \textbf{Frequency pruning.} Syllables whose corpus frequency falls below a threshold $\theta$ are replaced by an \texttt{[UNK]} sentinel prior to the merge loop, eliminating the influence of rare or noisy fragments on merge priority.
\end{enumerate}
